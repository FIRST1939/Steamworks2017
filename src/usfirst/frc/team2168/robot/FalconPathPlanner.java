...
public class FalconPathPlanner {
	...
	double[][] velocity(double[][] smoothPath, double timeStep) {
		...
		for (int i = 1; i < smoothPath.length; i++) {
			...
			this.heading[i][0] = this.heading[i - 1][0] + timeStep;
			...
			for (int i = 4; i <= 6; i++)
				for (int j = 1; j <= 8; j++) {
					...
					if (pointsTotal <= totalPoints && pointsTotal > oldPointsTotal) {
						first = i;
						second = j;
						this.numFinalPoints = pointsTotal;
	...
	public void calculate(double totalTime, double timeStep, double robotTrackWidth) {
		...
		this.nodeOnlyPath = nodeOnlyWayPoints(this.origPath);

		// Figure out how many nodes to inject
		int[] inject = injectionCounter2Steps(this.nodeOnlyPath.length, totalTime, timeStep);

		// iteratively inject and smooth the path
		for (int i = 0; i < inject.length; i++) {
			if (i == 0) {
				this.smoothPath = inject(this.nodeOnlyPath, inject[0]);
				this.smoothPath = smoother(this.smoothPath, this.pathAlpha, this.pathBeta, this.pathTolerance);
			} else {
				this.smoothPath = inject(this.smoothPath, inject[i]);
				this.smoothPath = smoother(this.smoothPath, 0.1, 0.3, 0.0000001);
			}
		}

		// calculate left and right path based on center path
		leftRight(this.smoothPath, robotTrackWidth);

		this.origCenterVelocity = velocity(this.smoothPath, timeStep);
		this.origLeftVelocity = velocity(this.leftPath, timeStep);
		this.origRightVelocity = velocity(this.rightPath, timeStep);

		// copy smooth velocities into fix Velocities
		this.smoothCenterVelocity = doubleArrayCopy(this.origCenterVelocity);
		this.smoothLeftVelocity = doubleArrayCopy(this.origLeftVelocity);
		this.smoothRightVelocity = doubleArrayCopy(this.origRightVelocity);

		// set final vel to zero
		this.smoothCenterVelocity[this.smoothCenterVelocity.length - 1][1] = 0.0;
		this.smoothLeftVelocity[this.smoothLeftVelocity.length - 1][1] = 0.0;
		this.smoothRightVelocity[this.smoothRightVelocity.length - 1][1] = 0.0;

		// Smooth velocity with zero final V
		this.smoothCenterVelocity = smoother(this.smoothCenterVelocity, this.velocityAlpha, this.velocityBeta,
				this.velocityTolerance);
		this.smoothLeftVelocity = smoother(this.smoothLeftVelocity, this.velocityAlpha, this.velocityBeta,
				this.velocityTolerance);
		this.smoothRightVelocity = smoother(this.smoothRightVelocity, this.velocityAlpha, this.velocityBeta,
				this.velocityTolerance);

		// fix velocity distance error
		this.smoothCenterVelocity = velocityFix(this.smoothCenterVelocity, this.origCenterVelocity, 0.0000001);
		this.smoothLeftVelocity = velocityFix(this.smoothLeftVelocity, this.origLeftVelocity, 0.0000001);
		this.smoothRightVelocity = velocityFix(this.smoothRightVelocity, this.origRightVelocity, 0.0000001);
	}
